...
export const crochetToolDefinitions: Tool[] = [
  {
    name: "crochet_validate_lesson",
    description:
      "Validate a crochet game lesson against the LeeWay Crochet Game Factory's deterministic content contract.",
    inputSchema: {
      type: "object",
      properties: {
        lesson: {
  ...
  },
  {
    ...
    description:
      "Diagnose count, tension, insertion, and turning-chain signals from an observed crochet attempt without claiming camera or sensor evidence that was not supplied.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Calculate measured stitches/rows per 10 cm and compare them with an optional target gauge.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Return lessons whose prerequisite lesson IDs are all complete, preserving the supplied lesson order.",
    inputSchema: {
      type: "object",
      properties: {
...
function finiteNumber(value: unknown, field: string, allowZero = false): number {
  if (
    typeof value !== "number" ||
...
function validateLesson(args: JsonObject): JsonObject {
  const lesson = args.lesson;
  if (!isObject(lesson)) throw new Error("lesson must be an object");

  const errors: string[] = [];
  if (typeof lesson.id !== "string" || !lesson.id.trim()) errors.push("lesson.id is required");
  if (typeof lesson.title !== "string" || !lesson.title.trim()) errors.push("lesson.title is required");
  if (!GAME_MODES.includes(lesson.game_mode as (typeof GAME_MODES)[number])) {
  ...
  if (!Array.isArray(lesson.steps) || lesson.steps.length === 0) {
    ...
    lesson.steps.forEach((step, index) => {
      ...
      if (!STITCH_ACTIONS.includes(step.action as (typeof STITCH_ACTIONS)[number])) {
...
function diagnoseAttempt(args: JsonObject): JsonObject {
  ...
  if (difference < 0 || missed > 0) diagnoses.push("possible-missed-stitch-or-edge-loss");
  if (difference > 0 || extra > 0) diagnoses.push("possible-extra-insertion-or-unintended-increase");
  if (args.turning_chain_present === false) diagnoses.push("turning-chain-missing");

  let tension: JsonObject = { state: "UNOBSERVED", sample_count: 0 };
  if (samples.length > 0) {
  ...
  if (diagnoses.length === 0) diagnoses.push("no-count-or-structure-anomaly-detected-from-supplied-fields");
  return {
...
function calculateGauge(args: JsonObject): JsonObject {
...
function recommendNextLesson(args: JsonObject): JsonObject {
  ...
  if (!Array.isArray(args.lessons)) throw new Error("lessons must be an array");
  const completed = new Set(args.completed_lesson_ids as string[]);
  ...
  for (const lesson of args.lessons) {
    ...
    if (!completed.has(lesson.id) && (lesson.prerequisite_ids as string[]).every((id) => completed.has(id))) {
...
export function isCrochetTool(name: string): boolean {
  return crochetToolDefinitions.some((tool) => tool.name === name);
}

export function executeCrochetTool(name: string, args: JsonObject = {}): string {
