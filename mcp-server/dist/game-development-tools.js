...
const arrayField = (description, itemType = "string") => ({
    type: "array",
    description,
    items: { type: itemType },
});
...
export const gameDevelopmentToolDefinitions = specs.map((spec) => ({
    name: spec.name,
...
export function isGameDevelopmentTool(name) {
    return specsByName.has(name);
}
function requireArguments(spec, args) {
...
export async function executeGameDevelopmentTool(name, args = {}) {
    const spec = specsByName.get(name);
    if (!spec)
        throw new Error(`Unknown game-development tool: ${name}`);
    validateArguments(spec, args);
    if (name === "game_plan_slice")
        return planSlice(args);
    ...
    try {
        ...
        catch { /* preserve non-JSON upstream evidence as text */ }
        const upstreamExecuted = Boolean(payload && typeof payload === "object" && payload.executed === true);
        return JSON.stringify({
            tool: name,
...
//# sourceMappingURL=game-development-tools.js.map