...
TAG: LEEWAY.SKILLS.SYNC.EXTERNAL.AGENT

WHAT = Synchronize approved general external Agent Skills into the canonical LeeWay Agent Skills repository
WHY = Keep research, code-understanding, prose, and video skill packages separate from the design-only synchronization receipt
WHO = Leeway Industries
WHERE = scripts/sync-external-agent-skills.ps1
WHEN = 2026
HOW = Read manifest, shallow-clone exact refs, copy declared paths, validate SKILL.md discovery, preserve license/provenance, write receipt
#>

[CmdletBinding()]
param(
    [string]$RepositoryRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path,
    [string]$ManifestPath = (Join-Path $PSScriptRoot 'external-agent-skills.json')
)
...
function Write-Step([string]$Message) {
    Write-Host "[LeeWay Agent Skill Sync] $Message"
}
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("leeway-agent-skill-sync-" + [Guid]::NewGuid().ToString('N'))
...
try {
    foreach ($source in $manifest.sources) {
        ...
        & git clone --depth 1 --branch $ref --single-branch "https://github.com/$repo.git" $cloneDir
        if ($LASTEXITCODE -ne 0) { throw "FAILED: git clone failed for $repo@$ref" }

        $commit = (& git -C $cloneDir rev-parse HEAD).Trim()
        if ($LASTEXITCODE -ne 0 -or -not $commit) { throw "FAILED: could not resolve source commit for $repo@$ref" }
        ...
        if (Test-Path -LiteralPath $upstream) { Remove-Item -LiteralPath $upstream -Recurse -Force }
        ...
        if ($skillFiles.Count -eq 0) { throw "FAILED: no SKILL.md discovered after syncing $id" }

        $licenseDir = Join-Path $destination 'provenance'
        New-Item -ItemType Directory -Path $licenseDir -Force | Out-Null
        foreach ($candidate in @('LICENSE','LICENSE.md','LICENSE.txt','NOTICE','NOTICE.md')) {
    ...
    $receiptPath = Join-Path $receiptDir 'external-agent-skills-sync.json'
    [ordered]@{
        schema = 'leeway.agent-skill-sync.receipt.v1'
        status = 'PASS'
    ...
    Write-Step "PASS: synchronized $($results.Count) general Agent Skill sources. Receipt: $receiptPath"
}
finally {
