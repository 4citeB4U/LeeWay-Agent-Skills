...
The Marketing Skills tool payload at the pinned E-drive commit is admitted under `tools/external/marketingskills`: 137 files, including 61 zero-dependency Node CLI scripts. Syntax validation proves parseability only. Individual API/MCP/SDK availability and credentials remain `UNVERIFIED` until exercised through an authorized adapter.
